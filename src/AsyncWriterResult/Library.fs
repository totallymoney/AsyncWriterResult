...
module Writer =
  ...
  let collect l =
    Writer <| fun () ->
      List.fold (fun (items, logs) (item, log) -> item::items, log::logs)
        ([], [])
        (List.map run l)
...
module WriterResult =
  ...
  let retn x =
    x |> Result.retn |> Writer.retn

  let map f =
    f |> Result.map |> Writer.map
  ...
  let apply f m =
    ...
    match r1, r2 with
    | Ok g, Ok h ->
      Writer <| fun () -> Ok (g h), logs1@logs2
    | Error e1, _ ->
      Writer <| fun () -> Error e1, logs1@logs2
    | _, Error e2 ->
      Writer <| fun () -> Error e2, logs1@logs2

  let collect list =
    let collectResult head tail =
      head |> Result.bind (fun h -> tail |> Result.bind (fun t -> Result.retn (h::t)))
    let folder (items, logs) (item, log) =
      collectResult item items, log@logs
    Writer <| fun () -> List.fold folder (Result.retn [], []) (List.map Writer.run list)

  let write log =
    Writer <| fun () -> Result.retn (), [log]


type AsyncWriterResult<'ok, 'error, 'log> =
  Async<Writer<'log list, Result<'ok, 'error>>>
...
module AsyncWriterResult =
  ...
  let retn x =
    x |> WriterResult.retn |> Async.retn

  let map f =
    f |> WriterResult.map |> Async.map
  ...
  let mapError e m = async {
    let! w = m
    let (r, logs) = Writer.run w
    return Writer <| fun () -> Result.mapError e r, logs }

  let private errMsg m (e:exn) =
    Error (sprintf "%s: %s" m e.Message)
