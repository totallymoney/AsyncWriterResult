...
module TaskWriterResultCE =
    ...
    module TaskWriterResultBuilderExtensions =
        type TaskWriterResultBuilder with
            ...
            member __.Source(x: Async<'t>) =
                x |> Async.StartAsTask |> Task.map WriterResult.retn
    ...
    module TaskWriterResultBuilderExtensionsHighPriority =
        type TaskWriterResultBuilder with
            ...
            member __.Source(x: Async<Result<'a, 'b>>) =
                x |> Async.StartAsTask |> Task.map Writer.retn

            member __.Source(x: Async<Writer<'w, 't>>) =
                x |> Async.StartAsTask |> TaskWriter.map Result.retn
