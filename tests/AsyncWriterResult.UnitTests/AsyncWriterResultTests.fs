...
let ceTests =
    [ test "and! should run in parallel" {
      ...
      test "can be used in a for loop" {
          ...
          let _, actualWritten = aw |> Async.RunSynchronously |> Writer.run
      ...
      test "can use if-return" {
          ...
          let _, actualWritten = aw true |> Async.RunSynchronously |> Writer.run
...
[<Tests>]
let tests =
    testList "AsyncWriterResult" [ testList "Helpers" []; testList "CE" ceTests; testList "Operators" [] ]
